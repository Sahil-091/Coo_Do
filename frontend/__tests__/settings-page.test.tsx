  ...
  internalApiFetch: vi.fn(async (path: string) => {
    if (path.endsWith("/consents")) {
      return [
        ...
        { consent_type: "activity_alerts", granted: false, updated_at: null },
...
describe("Settings / Privacy Center page", () => {
  ...
  test("renders account info, all 5 consent toggles, and has no axe violations", async () => {
    ...
    // All 5 consent types render as switches, reflecting the mocked state.
    const switches = screen.getAllByRole("switch");
    // 5 consent toggles + 2 privacy-visibility toggles = 7
    ...
    expect(screen.getByRole("switch", { name: /local activity alerts/i })).toHaveAttribute(
      "aria-checked",
      "false"
    );
