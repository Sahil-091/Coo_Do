  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="mt-2 flex items-center gap-3">
        ...
        <Button type="button" disabled={!selected || isPending || !isOnline} onClick={onSubmit} className="flex-1">
          {isPending ? "One sec…" : "See what might help"}
        </Button>
      </div>
      {!isOnline && <p role="status" className="text-sm text-ink-muted">Reconnect to save this check-in and get a suggestion. Your choices stay only in this open screen.</p>}
    </div>
  );
}
