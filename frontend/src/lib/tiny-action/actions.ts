...
interface SuggestionApiResponse {
  ladder: { id: string; difficulty_level: number; title: string; description: string; voice_message: string }[];
...
function mapSuggestion(api: SuggestionApiResponse): SuggestionResult {
  return {
    ladder: api.ladder.map((r) => ({
      ...
      voiceMessage: r.voice_message,
    })),
    suggestedAttemptId: api.suggested_attempt_id,
  };
}

export async function getSuggestionAction(checkinId?: string): Promise<SuggestionResult | { error: string }> {
  const user = await requireOnboardedUser();
  try {
    const response = await internalApiFetch<SuggestionApiResponse>(
      `/internal/users/${user.userId}/tiny-actions/suggestion${
        checkinId ? `?checkin_id=${encodeURIComponent(checkinId)}` : ""
      }`
    );
